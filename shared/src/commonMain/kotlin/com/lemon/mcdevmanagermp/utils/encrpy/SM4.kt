...
internal object SM4 {
    ...
    fun encrypt(plaintext: String, keyHex: String): String {
        ...
        val padded = pkcs7Pad(inputBytes)
        val roundKeys = expandKey(keyBytes)

        val output = ByteArray(padded.size)
        for (i in padded.indices step 16) {
            val block = padded.copyOfRange(i, minOf(i + 16, padded.size))
            val encrypted = encryptBlock(block, roundKeys)
    ...
    private fun encryptBlock(block: ByteArray, rk: IntArray): ByteArray {
        val x = IntArray(36)
        for (i in 0 until 4) {
            x[i] = bytesToInt(block, i * 4)
        }
        ...
        return ByteArray(16) { i ->
            val wordIdx = 35 - i / 4
            val byteIdx = i % 4
            (x[wordIdx] ushr (24 - byteIdx * 8)).toByte()
        }
    }

    // 合成置换 T = L ∘ τ (用于加密轮函数)
    private fun t(input: Int): Int {
        val substituted = tau(input)
        // L(B) = B ⊕ (B<<<2) ⊕ (B<<<10) ⊕ (B<<<18) ⊕ (B<<<24)
        return substituted xor rotateLeft(substituted, 2) xor
                rotateLeft(substituted, 10) xor rotateLeft(substituted, 18) xor rotateLeft(substituted, 24)
    }

    // 合成置换 T' = L' ∘ τ (用于密钥扩展)
    private fun tPrime(input: Int): Int {
        val substituted = tau(input)
        // L'(B) = B ⊕ (B<<<13) ⊕ (B<<<23)
        return substituted xor rotateLeft(substituted, 13) xor rotateLeft(substituted, 23)
    }

    // 非线性变换 τ: S盒字节替换
    private fun tau(input: Int): Int {
        return (SBOX[(input ushr 24) and 0xFF] shl 24) or
                (SBOX[(input ushr 16) and 0xFF] shl 16) or
                (SBOX[(input ushr 8) and 0xFF] shl 8) or
                SBOX[input and 0xFF]
    }
